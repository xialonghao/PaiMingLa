    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $key => $row) {
                ...
                $row->visible(['id','order_sn','create_time','ranking','money','status','amount','keyword_id']);
                $row->visible(['channel']);
                ...
                $list[$key]['amount'] = $this->model->where(['keyword_id'=>$row['keyword_id']])->sum('money');
            }
            $list = collection($list)->toArray();
            $result = array("total" => $total, "rows" => $list);
            return json($result);
        }
        !empty($this->request->route('ids')) ? $this->assign('projcet_id',$this->request->route('ids')) : $this->assign('projcet_id','');
